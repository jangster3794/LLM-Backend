...
# Define blueprint
openai_llm_bp = Blueprint('openai_llm', __name__)
...
@openai_llm_bp.route("/single-query", methods=["POST"])
@cross_origin()
def single_query():
...
@openai_llm_bp.route("/convo-chain", methods=["GET"])
@cross_origin()
def get_convo_chain():
...
@openai_llm_bp.route("/convo-chain", methods=["POST"])
@cross_origin()
def convo_chain():
    question = json.loads(request.data).get("question")
    if not question:
...
@openai_llm_bp.route("/most-common-disease-per-country")
@cross_origin()
...
@openai_llm_bp.route("/chain-country-disease-symptoms")
@cross_origin()
